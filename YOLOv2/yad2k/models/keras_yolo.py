...
class BilinearUpSampling(layers.Layer):
    def __init__(self, outHeight, outWidth):
        ...
        # print(outWidth)
        # print(outHeight)
    def call(self, InputTensro):
        return tf.image.resize(
            InputTensro,
            method=tf.image.ResizeMethod.BILINEAR,
            size=(self.outHeight, self.outWidth)
        )
...
def space_to_depth_x2(x):
    ...
    import tensorflow as tf
    block_size = 2
    height_pad = (block_size - x.shape[1] % block_size) % block_size
    width_pad = (block_size - x.shape[2] % block_size) % block_size
    padding = tf.constant([[0, 0], [0, height_pad], [0, width_pad], [0, 0]])
    padded_input = tf.pad(x, padding)
    
    # Space-to-depth transformation
    output = tf.nn.space_to_depth(padded_input, block_size=2)
    return output
...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    # print(conv21_reshaped.shape)
    # print(conv20.shape)
    BilinearUpSampling_obj = BilinearUpSampling(conv20.shape[1], conv20.shape[2])
    conv21_reshaped = BilinearUpSampling_obj.call(conv21_reshaped)
    # print(conv21_reshaped.shape)
    # print(conv20.shape)
